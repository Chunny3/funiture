...
<head>
  ...
  <!-- 無影響欄位 Custom fonts for this template -->
...
<div class="container mt-5">
  ...
  <div class="card shadow max-width1200 mx-auto mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-white">設定個人基本資料</h6>
    </div>
            ...
            <?php foreach ($rowsLevel as $rowLevel): ?>
              <option value="<?= $rowLevel["id"] ?>" <?php
                if (
                  ...
                  echo "selected";
                ?>>
                <?= htmlspecialchars($rowLevel["name"]) ?>
              </option>
            <?php endforeach; ?>
          </select>
        </div>
  ...
  <script>
    ...
    const defaultCity = "<?= htmlspecialchars($row['city'] ?? "") ?>";
    const defaultArea = "<?= htmlspecialchars($row['area'] ?? "") ?>";
